...
class RouteTrieSpec extends Specification {
  "RouteTrie" should {
    "match URLs" in {
      ...
      tree.get("/user") must_== (1, None)
      tree.get("/user/12") must_== (5, Some(OpenHashMap("id" -> "12")))
      tree.get("/user/12/") must_== (5, Some(OpenHashMap("id" -> "12")))
      tree.get("/user/12//") must_== None
      tree.get("/user/12/x/333") must_== (12, Some(OpenHashMap("id" -> "12", "xid" -> "333")))
      tree.get("/user/about") must_== (3, None)
      tree.get("/ua/kek") must_== (7, None)
      tree.get("user") must_== None
      tree.get("/f/тест/DFSD©Δ§ß∞¢") must_== (9, Some(OpenHashMap("file" -> "тест/DFSD©Δ§ß∞¢/")))
      tree.get("/урл/тест/DFSD©Δ§ß∞¢") must_== (10, None)
      tree.get("/no_url") must_== (11, Some(OpenHashMap("kek" -> "no_url")))
    }
